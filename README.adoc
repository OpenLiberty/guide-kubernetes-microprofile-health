...
:page-seo-title: Checking the health of microservices using Kubernetes readiness and liveness probes
:page-seo-description: A tutorial and example on how to report health status of microservices using MicroProfile Health Check and check them with Kubernetes readiness probes and liveness probes
...
health status to a defined endpoint. If a service reports `UP`, then it's available. If 
...
or make HTTP requests. MicroProfile Health exposes a readiness and liveness endpoint on 
...
If more than 90% of the maxmimum memory is used, a status of `DOWN` will be returned. 

The health checks for the `system` microservice have already been implemented. The `system`
microservice has been set up to become unhealthy for 60 seconds when a specific endpoint is called. 
...
The readiness probes are configured to poll the [hotspot=25 hotspot=64]`/health/ready` endpoint. The readiness probe 
determines the READY status of the container as seen in the `kubectl get pods` output. The 
[hotspot=27 hotspot=66]`initialDelaySeconds` field defines how long the probe should wait before it 
starts to poll so the probe does not start making requests before the server has started. 
The [hotspot=29 hotspot=68]`failureThreshold` option defines how many times the probe should fail 
before the state should be changed from ready to not ready. The [hotspot=28 hotspot=67]`periodSeconds` 
option defines how often the probe should poll the given endpoint.

The liveness probes are configured to poll the [hotspot=32 hotspot=71]`/health/live` endpoint. 
The liveness probe determines when a container needs to be restarted. Similar to the readiness
probes, the liveness probes also define [hotspot=34 hotspot=73]`initialDelaySeconds`, 
[hotspot=36 hotspot=75]`failureThreshold`, and [hotspot=35 hotspot=74]`periodSeconds`.
