...
{kube} provides liveness and readiness probes that are used to check the health of your containers. These probes can check certain files in your containers, check a TCP socket, or make HTTP requests. MicroProfile Health exposes a health endpoint on your microservices. {kube} polls the endpoint as specified by the liveness and readiness probes.
...
Next, you will see the pods start to become ready again. You will also notice the `ping` pod is no longer ready, since the readiness probe failed since it might no longer reach the service `name-service`.
...
The tests check that `name` responds with a container name. They also check that `ping` responds with `pong` on success and it gracefully handles a bad response.
