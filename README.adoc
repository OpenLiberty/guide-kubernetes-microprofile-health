...
[role="code_command hotspot=20-50", subs="quotes"] 
---- 
...
[role="code_command hotspot=1-90", subs="quotes"] 
---- 
...
[hotspot=27]`initialDelaySeconds` field defines how long the probe should wait before it 
starts to poll so the probe does not start making requests before the server has started. 
The [hotspot=29]`failureThreshold` option defines how many times the probe should fail 
before the state should be changed from ready to not ready. The [hotspot=28]`periodSeconds` 
...
You have used MicroProfile Health to create an endpoint that reports on your microservice's 
status. Then, you observed how {kube} uses the `/health` endpoint to keep your 
microservices running smoothly.
