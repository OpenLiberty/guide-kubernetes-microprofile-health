...
An endpoint has been provided under the `name` microservice to set it to an unhealthy state in the health check. The unhealthy state will cause the readiness and liveness probe to fail. Invoke this endpoint by making a POST request to `{name-api}/unhealthy`. Run the following command to view the state of the pods:
