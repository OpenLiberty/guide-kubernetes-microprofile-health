...
The [hotspot=threshold1 hotspot=threshold3]`failureThreshold` option option defines
how many times the probe fails before the state changes from ready to not ready.
The [hotspot=timeout1 hotspot=timeout3]`timeoutSeconds` option defines
how many seconds before the probe times out.
The [hotspot=period1 hotspot=period3]`periodSeconds` option defines
how often the probe should poll the given endpoint.
...
Set the image pull policy to **Always** and remove the **nodePort** fields so that the ports are automatically generated:
...
With the readiness probe, you can be certain the pod accepts traffic
only when the microservice is fully started.
...
// Cloud hosted guide instruction
ifdef::cloud-hosted[]
An endpoint is provided under the **system** microservice that sets it to an unhealthy
state in the health check. The unhealthy state causes the readiness probe to fail.
...
pod becomes unhealthy. You may see the same pod twice because there's a delay 
...
From a terminal, run the following POST request to change the first pod to a not-ready state.
```
SYSTEM_NODEPORT=`kubectl get -o jsonpath="{.spec.ports[0].nodePort}" services system-service`
curl -X POST http://localhost:$SYSTEM_NODEPORT/system/unhealthy
```
...
// Cloud-hosted guide instruction
ifdef::cloud-hosted[]
Go back to the terminals where you started the port forwarding for the
**system** service and the **inventory** service.
Press **CTRL+C** to stop the port forwardings.
endif::[]
