...
starts to poll so the probe does not start making requests before the server has started.  The [hotspot=period1 hotspot=period3]`periodSeconds` 
option defines how often the probe should poll the given endpoint. The [hotspot=timeout1 hotspot=timeout3]`timeoutSeconds` 
option defines how many seconds before the probe times out. The [hotspot=threshold1 hotspot=threshold3]`failureThreshold` option defines how many times the probe fails 
