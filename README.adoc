...
Make a request to the `{system-api}/properties` URL with the following command:

```
curl {system-api}/properties
```
{: codeblock}
...
ifdef::cloud-hosted[]
Wait until the `system` pod is ready again.
Make two POST requests to `{system-api}/unhealthy` with the following command:
