...
before the state changes from ready to not ready. The [hotspot=delay1 hotspot=delay3]`initialDelaySeconds` field defines how long the probe waits before it 
starts to poll so the probe does not start making requests before the server has started. The [hotspot=timeout1 hotspot=timeout3]`timeoutSeconds` 
...
[hotspot=threshold2 hotspot=threshold4]`failureThreshold`, [hotspot=timeout2 hotspot=timeout4]`timeoutSeconds`,
