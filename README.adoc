...
Otherwise, this annotation is ignored.
This startup check verifies that the cpu memory usage is below 95% of the maximum memory.
If more than 95% of the maximum memory is used, a status of `DOWN` is returned. 
...
You will configure {kube} startup, liveness and readiness probes. Startup probes determines
