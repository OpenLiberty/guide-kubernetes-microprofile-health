...
The [hotspot=threshold2 hotspot=threshold4]`failureThreshold` option defines how many times the probe fails 
before the state changes from ready to not ready. The [hotspot=delay2 hotspot=delay4]`initialDelaySeconds` field defines how long the probe waits before it 
starts to poll so the probe does not start making requests before the server has started. The [hotspot=timeout2 hotspot=timeout4]`timeoutSeconds` 
option defines how many seconds before the probe times out. The [hotspot=period2 hotspot=period4]`periodSeconds` 
...
Similar to the liveness probes, the readiness probes also define [hotspot=delay1 hotspot=delay3]`initialDelaySeconds`,
[hotspot=threshold1 hotspot=threshold3]`failureThreshold`, [hotspot=timeout1 hotspot=timeout3]`timeoutSeconds`,
and [hotspot=period1 hotspot=period3]`periodSeconds`.
