...
----
include::finish/inventory/src/main/java/io/openliberty/guides/inventory/InventoryHealth.java[tags=**;]
----
...
and one `inventory` pod, later you'll observe their behaviour as the `system` pods become unhealthy.
...
Then, the `system` pods will start to recover.
